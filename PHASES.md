...
- Gmail `new_labeled_email` (fires when a label is applied to a message) was built, verified directly against Pipedream's actual source (`components/gmail/sources/new-labeled-email/`), then **removed** (2026-09-10) — with no per-instance label filter (`src/api/trigger_routes.ts` has no per-instance config field yet, same gap `new_email_matching_search` hit), it fired on EVERY label change on EVERY message, which in practice mostly duplicated what `new_email`/`new_starred_email` already report (confirmed live: starring a message fires it TWICE, once per system label Gmail applies — `STARRED` + `YELLOW_STAR`). A fabricated "new_label" (label *creation*) trigger was built first and then removed too — Gmail has no event feed for label creation at all (only message-level `labelAdded`/`labelRemoved`), confirmed by exhaustively listing all 5 of Pipedream's real Gmail sources (none of them is label-creation either).
- TODO: if a labeled-email trigger is wanted back, it needs a per-instance label filter (subscribe-time `label_ids: string[]`) before it's worth re-adding — otherwise it's just a noisier `new_email`. Needs the same per-instance trigger config field `new_email_matching_search`'s TODO already calls for.
...
- ✅ Slack bot scopes (`src/components/slack/app.ts`) grew from 13 to 20 (2026-09-10): `groups:read`/`groups:history`/`groups:write`, `im:read`/`im:history`, `mpim:read`/`mpim:history` — closes the gap where `message.groups`/`message.im`/`message.mpim` (private channels/DMs/group DMs, already documented in `.env.example`'s Event Subscriptions setup) could never actually fire, and `createChannel.ts`'s `is_private: true` 403. Cross-referenced against a real working bot's production scope set in the sibling project `/home/gr/projects/backend` (`core/config/slack.py`). An existing connection authorized before this change needs to reconnect — a scope change needs a fresh OAuth grant.
- ✅ Inbound Slack Events API dedup (2026-09-10): Slack retries the same event (same `event_id`) if our endpoint doesn't ack within 3s or returns non-2xx — `src/components/slack/webhooks/events.ts`'s `isFirstDelivery()` uses a Redis `SET NX EX` on `slack:event:dedup:{event_id}` (10 min TTL) to make a repeat delivery a no-op instead of double-firing every subscriber's `webhook_url`. Same pattern as the sibling project `/home/gr/projects/backend` (`slack_gateway_routes.py`). Verified live: same event_id set twice against real Redis returns `OK` then `null`.
- ⚠️ Slack triggers switched from bot-scoped to **user-scoped** Events API delivery (2026-09-10, see `src/components/slack/TRIGGER_USER_SCOPED_REFACTOR.md`) — matches Pipedream's real, verified behavior on the nose: Pipedream's own trigger fired from a channel with zero Pipedream bot in `conversations.members`, confirmed as Slack's real "Subscribe to events on behalf of users" mode (`docs.slack.dev/apis/events-api/`), not a legacy artifact. `app.ts`'s `extraAuthorizeParams.user_scope` grew to include `channels:history`/`groups:history`/`im:history`/`mpim:history`/`reactions:read` (+ paired `:read` scopes). **Net effect: the bot no longer needs to be a member of a channel to receive its events at all** — removes the join/invite requirement entirely for triggers, same as Pipedream. `channels:join` bot scope and the `join_channel` action were deleted outright (decided by the user, not kept as a utility) since self-join was their only reason to exist. Actions (`post_message`, `get_channel_history`, etc.) are untouched — still bot-token, still need `chat:write.public`/an explicit invite for private channels, exactly as before; only trigger event *receipt* moved. `bunx tsc --noEmit` and `bun test` (105 pass) both clean. **Not yet verified live** — needs a real OAuth reconnect through the new user scopes and a real event from a channel the bot was never added to (see the refactor doc's "what you need to do to finish this" section). Flip this line to ✅ once that's confirmed.
