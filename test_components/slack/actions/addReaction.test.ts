...
describe("addReaction", () => {
  test("sends `timestamp`, not `ts`, on the wire (reactions.* naming quirk)", async () => {
    ...
    const result = await runAction(addReaction, makeBotOnlyConnection(), { channel: "C1", ts: "1.2", name: "thumbsup" });
  ...
  test("surfaces already_reacted", async () => {
    ...
    await expect(
      runAction(addReaction, makeBotOnlyConnection(), { channel: "C1", ts: "1.2", name: "thumbsup" }),
