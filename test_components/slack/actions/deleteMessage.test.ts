...
describe("deleteMessage", () => {
  test("deletes a message", async () => {
    ...
    const result = await runAction(deleteMessage, makeBotOnlyConnection(), { channel: "C1", ts: "1.2" });
  ...
  test("surfaces ok:false errors", async () => {
    ...
    await expect(runAction(deleteMessage, makeBotOnlyConnection(), { channel: "C1", ts: "1.2" })).rejects.toThrow(
      /message_not_found/,
    );
  });
});
