...
describe("inviteUserToChannel", () => {
  test("joins multiple user ids with a comma on the wire", async () => {
    ...
    const result = await runAction(inviteUserToChannel, makeBotOnlyConnection(), { channel: "C1", users: ["U1", "U2"] });
  ...
  test("surfaces already_in_channel", async () => {
    ...
    await expect(
      runAction(inviteUserToChannel, makeBotOnlyConnection(), { channel: "C1", users: ["U1"] }),
