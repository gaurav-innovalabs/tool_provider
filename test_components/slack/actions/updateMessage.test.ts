...
describe("updateMessage", () => {
  test("edits an existing message", async () => {
    ...
    const result = await runAction(updateMessage, makeBotOnlyConnection(), { channel_id: "C1", ts: "1.2", text: "edited" });
  ...
  test("surfaces the identity-mismatch error verbatim", async () => {
    ...
    await expect(
      runAction(updateMessage, makeBotOnlyConnection(), { channel_id: "C1", ts: "1.2", text: "edited" }),
